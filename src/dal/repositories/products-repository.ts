...
export const getAllProducts = async () =>
    (await Product.find({})).map(foundProduct => new ProductDto(foundProduct));
...
export const deleteProduct = async ({ id }) => {
    ...
    if (result.n !== 1) {
        logger.warn(
            `unable to delete product by id: '${id}'. result is: \n${JSON.stringify(result)}`,
        );
...
export const updateProduct = async (id, updateObject) => {
    ...
    if (!result.ok) {
        logger.warn(
            `unable to update product by id: '${id}'. result is: \n${JSON.stringify(result)}`,
        );
...
export const changeProductsAmount = async (checkedAmounts: [{ id: string; amount: number }]) => {
    const results = await Promise.all(
        checkedAmounts.map(async ({ id, amount }) => {
